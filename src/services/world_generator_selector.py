...
class WorldGeneratorSelector:
    ...
    def select_rsg_world_generator(self, window_title: str) -> BaseWorldGenerator:
        ...
        if major_version == 1.16:
            ...
            raise UnsupportedVersionError(f"RSG world generation for version {major_version} is not supported yet")
        else:
            raise UnsupportedVersionError(
                f"RSG world generation forversion {major_version} is not supported and probably won't be in the future"
            )
